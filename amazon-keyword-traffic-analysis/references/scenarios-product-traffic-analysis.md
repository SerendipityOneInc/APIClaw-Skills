...
|---|---|---|
| Current ASIN-wide structure/change or change drivers | `product-traffic-structure-profile` | Stop when the returned current/previous modules answer the question; do not reconstruct or duplicate them from one traffic-term page |
| ASIN-wide multi-week trend or trajectory | `product-traffic-trend-profile` for the supported four-week server conclusion; `product-traffic-trend` when the requested deliverable requires raw weekly points or a period longer than four weeks | Do not add the raw series to a four-week profile answer unless weekly detail is required; keep every raw request within the range contract in `reference.md` |
| Current traffic terms, sources, contributors, or one named term's current visibility/placement | `keywords/product-traffic-terms` | Use the same route for the user's ASIN or a competitor ASIN. For a named term, select only its exact returned row after retrieval; add `search-results` only when the question explicitly requires that keyword's observed SERP placement. |
| Named ASIN × keyword movement or trend | `product-traffic-terms-trend` | Add only the market, SERP, product-event, or aggregate evidence required to resolve the named movement question |
...
If a structure or trend profile is empty or unavailable, do not reconstruct it from traffic-term rows. If `product-traffic-terms` is unavailable, do not make keyword-level product traffic-source claims. `detail`, `market-profile`, and `search-results` cannot replace the missing traffic contract.
...
|---|---|
| Aggregate structure | Structure-profile channel shares, impression points, term counts, and current-period scope |
| Aggregate change | Compatible current/previous structure-profile evidence, gained/lost terms, and returned change drivers |
| Aggregate trend | Trend-profile component conclusions or aligned raw weekly ASIN-level traffic/coverage points |
...
| Named-term trend | Traffic-term trend placement, traffic, keyword metrics, ad activity, and ASIN snapshots across aligned returned periods |
...
- Named ASIN × keyword trend with product, placement, traffic, demand, and event evidence kept at their returned grains.
- ASIN-wide four-week trend conclusion, with raw weekly points only when the requested conclusion needs them.
...
|---|---|---|---|
| 1A. Product traffic health overview | ASIN + broad traffic analysis/health request, current aggregate structure question, or ASIN-wide change/trend question without a named keyword | The matching aggregate evidence: `product-traffic-structure-profile` for current/previous structure and drivers; `product-traffic-trend-profile` for a four-week server trajectory; or raw `product-traffic-trend` when the user explicitly requests weekly points, detail omitted by the metric, or a period longer than four weeks. Interpret returned fields through `traffic-observation-semantics.md`; use multiple aggregate capabilities only when the requested conclusion explicitly requires their distinct scopes. | Describe returned structure/change, profile trend, or raw weekly movement within its resolved scope and health boundaries. Do not infer omitted modules, per-keyword contribution, unsupported periods, or cause. |
...
- Stage 1A follows metric-first access for a four-week trend conclusion. Preserve item/row status, coverage, resolved period/window scope, returned modules, fields, and evidence. Do not reconstruct the profile from traffic rows. Use raw `product-traffic-trend` only for required weekly points, detail omitted by the metric, or an explicitly requested period longer than four weeks; do not stretch the four-week profile across that longer period or relabel an Agent reading of raw points as the server profile.
- Keep a raw trend request within the maximum range owned by `reference.md`. If the user's requested period exceeds that contract, state the unsupported interval and request or offer a narrower range; do not silently truncate it or issue an out-of-contract call.
...
- Stage 2 requires one named keyword and a relevant range. Do not call the traffic-term trend for an unnamed ASIN-wide question or use a current traffic-term row as a movement series.
- Separate external keyword movement from product-side movement. Keyword trend evidence can explain market context; it does not by itself establish that the product gained or lost visibility. Product traffic-term trend evidence can show product-side movement; it does not by itself establish market demand or cause.
